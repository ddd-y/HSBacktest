...
class momentum_20_data
{
private:
	std::vector<double> momentum_20_values; // 存储每只股票调仓日的最近20个交易日动量值
public:
	double get_momentum_20(const int index) const
	{
		return momentum_20_values[index];
	}

	std::vector<double>& get_momentum_20_values()
	{
		return momentum_20_values;
	}

	momentum_20_data(int size) : momentum_20_values(size, 0.0)
	{}
};
...
public:
	...
	momentum_20(momentum_20_data& data) : momentum_20s(data.get_momentum_20_values())
	{}
};
