...
{
private:
	std::vector<double> ep_ratio_values; // 存储每只股票调仓日的盈利收益率(EP)值
public:
	double get_ep_ratio(const int index) const
	{
		return ep_ratio_values[index];
	}

	std::vector<double>& get_ep_ratio_values()
	{
		return ep_ratio_values;
	}

	ep_ratio_data(int size) : ep_ratio_values(size, 0.0)
	{}
};
...
public:
	...
	*/
	...
	/*
	* @brief 构造函数
	* @param data ep_ratio_data引用
	*/
	ep_ratio(ep_ratio_data& data) : ep_ratios(data.get_ep_ratio_values())
	{}
};